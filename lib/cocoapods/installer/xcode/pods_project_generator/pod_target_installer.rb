        ...
        class PodTargetInstaller < TargetInstaller
          ...
          def install!
            UI.message "- Installing target `#{target.name}` #{target.platform}" do
              ...
              if target.defines_module?
                ...
                create_umbrella_header(native_target) do |generator|
                  generator.imports += library_file_accessors.flat_map do |file_accessor|
                    header_dir = if !target.type.framework? && dir = file_accessor.spec_consumer.header_dir
                                   Pathname.new(dir)
                    end
              ...
              if target.type.dynamic_framework?
          ...
          def add_files_to_build_phases(native_target, test_native_targets, app_native_targets)
            target.file_accessors.each do |file_accessor|
              ...
              next unless target.type.dynamic_framework?
          ...
          def add_header(build_file, public_headers, private_headers, native_target)
            ...
            acl = if !target.type.framework? # Headers are already rooted at ${PODS_ROOT}/Headers/P*/[pod]/...
                    'Project'
            ...
            if target.type.framework? && header_mappings_dir && acl != 'Project'
